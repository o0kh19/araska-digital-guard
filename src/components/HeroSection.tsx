...
const HeroSection = () => {
  ...
  const heroTitle = t("hero.title") as [string, string];
  const services = t("hero.services") as Array<{
    title: string;
    subtitle: string;
    items: string[];
    Icon: typeof ShieldCheck | typeof Crosshair | typeof Briefcase | typeof GraduationCap;
  }>;
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center mb-16">
          <motion.div variants={container} initial="hidden" animate="show">
            ...
            >
              {heroTitle[0]}
              <br />
              <span style={{ color: `hsl(${RED})` }}>{heroTitle[1]}</span>
            </motion.h1>
            ...
            <motion.div variants={item} className="flex flex-wrap items-center gap-6">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-600">
                <span className="font-medium">{t("hero.builtOn")}</span>
                {((t("hero.frameworks") as string[]) || []).map((framework, index, list) => (
                  <span key={framework} className="inline-flex items-center gap-2">
                    <span className="font-bold text-slate-900">{framework}</span>
                    {index !== list.length - 1 && <span className="text-slate-400">•</span>}
                  </span>
                ))}
              </div>
            </motion.div>
            ...
            return (
              ...
              >
                ...
                <ul className="space-y-2.5">
                  {s.items.map((it) => (
                    <li key={it} className="flex items-start gap-2.5 text-slate-700 text-sm">
                      <span
